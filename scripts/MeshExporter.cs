...
public class MeshExporter
{
    ...
    public static byte[] ExportObj(ArrayMesh mesh)
    {
        ...
        // TODO: deduplication using the dictionary method
        ...
        for (int i = 0; i < mesh.GetSurfaceCount(); i++)
        {
            ...
            for (int j = 0; j < vertices.Length; j++)
                WriteString("v " + vertices[j].X + " " + vertices[j].Y + " " + vertices[j].Z + "\nvn " + normals[j].X + " " + normals[j].Y + " " + normals[j].Z + "\n", ref data);

            WriteString("\n", ref data);
            for (int j = 0; j < indices.Length - 2; j += 3)
                WriteString("f " + (indices[j] + v_offset) + "//" + (indices[j] + v_offset) + " " + (indices[j + 1] + v_offset) + "//" + (indices[j + 1] + v_offset) + " " + (indices[j + 2] + v_offset) + "//" + (indices[j + 2] + v_offset) + "\n", ref data);
            WriteString("\n\n", ref data);
